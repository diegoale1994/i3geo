//+$trad(1,i3GEOF.convertemapaKml.dicionario)+
i3GEOF.converteMapaKml.dicionario = {
	'ajuda' : [ {
		pt : "Voc&ecirc; pode utilizar os endere&ccedil;os para visualizar o mapa atual em softwares que aceitam o formato kml,",
		en : "",
		es : "Usted puede utilizar las direcciones para visualizar datos en softwares que acepten el formato kml"
	} ],
	'ajuda2' : [ {
		pt : "O endere&ccedil;o de acesso &eacute; tempor&aacute;rio, ficando dispon&iacute;vel por determinado per&iacute;odo de tempo conforme definido pelo administrador do i3Geo.",
		en : "",
		es : "La direcci&oacute;n de acceso es temporal, queda disponible por un periodo de tiempo determinado por el administrador de i3Geo. "
	} ],
	'ajuda3' : [ {
		pt : "para mais detalhes sobre como usar o link kml no Google Earth.",
		en : "",
		es : "para m&acute;s detalles sobre c&oacute;mo usar el link km en Google Earth."
	} ],
	'wms' : [ {
		pt : "Kml baseado em um servico WMS:",
		en : "",
		es : "Kml basado en un servicio WMS"
	} ],
	'ajuda4' : [ {
		pt : "Voc&ecirc; pode tamb&eacute;m utilizar o link abaixo para mostrar a &aacute;rvore completa de temas no GoogleEarth (incluindo dados vetoriais)",
		en : "",
		es : "Usted puede tambi&eacute;n utilizar el siguiente link (abajo) para mostrar el &aacute;rbol completo de temas en Google Earth"
	} ]
};
